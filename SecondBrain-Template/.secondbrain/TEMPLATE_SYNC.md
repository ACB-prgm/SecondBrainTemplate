...
Only managed paths may be overwritten from the template. User-created subject folders, `manifest.md`, and `index/*.json` are user data after repo creation and must not be overwritten by template sync unless explicitly listed in managed paths and the user approves.
